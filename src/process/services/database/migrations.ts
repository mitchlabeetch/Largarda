...
const migration_v18: IMigration = {
  ...
  up: (db) => {
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        user_id TEXT NOT NULL,
        name TEXT NOT NULL,
        type TEXT NOT NULL CHECK(type IN ('gemini', 'acp', 'codex', 'openclaw-gateway', 'nanobot', 'remote', 'dispatch')),
        extra TEXT NOT NULL,
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'idle', 'finished', 'failed', 'cancelled')),
        source TEXT,
        channel_chat_id TEXT,
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
