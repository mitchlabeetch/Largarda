...
import { AcpError, type AcpErrorCode } from './AcpError';
import { extractAcpError, formatUnknownError } from './errorExtract';
...
export function normalizeError(error: unknown): AcpError {
  ...
  if (acpPayload) {
    ...
    if (acpPayload.message.toLowerCase().includes('auth_required')
