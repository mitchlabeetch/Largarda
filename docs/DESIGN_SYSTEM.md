# Largo Design System — *Mint Whisper*
...
### Fraîcheur — *Invigorating Freshness*
...
| **Tertiary**   | *(bg-6)*    | *(bg-6)*    | `--bg-6`            | `text-t-tertiary`  | Hints, timestamps      |
...
| **1**       | *(bg-3)*    | *(bg-3)*    | `--bg-3`           | `border-b-1`     | Secondary borders    |
| **2**       | *(bg-4)*    | *(bg-4)*    | `--bg-4`           | `border-b-2`     | Prominent borders    |
| **3**       | *(bg-5)*    | *(bg-5)*    | `--bg-5`           | `border-b-3`     | Heavy borders        |
...
@keyframes team-tab-breathe {
  0%, 100% { opacity: 1; /* base state */ }
  50%      { opacity: 0.85; /* subtle fade with box-shadow glow */ }
}
/* Duration: 2s cycle */
```
...
| `--text-disabled` (#c2beb5) on `--bg-base` (#fdfcf9) | **1.8:1**  | — ⚠️ *Intentionally low for disabled state* |
...
@media (prefers-color-scheme: dark) {
  .my-component { background: #1a1a1a; }
}
```
...
```tsx
<div className="p-4 gap-6">           {/* Generous internal + section spacing */}
...
```tsx
<div className="p-1 gap-1">           {/* Cramped — violates Respiration */}
...
```css
.card { border-radius: var(--radius-lg); }   /* 16px — friendly, rounded */
.badge { border-radius: var(--radius-sm); }   /* 8px — compact elements  */
...
```css
.card { border-radius: 2px; }    /* Too sharp for Mint Whisper */
.card { border-radius: 30px; }   /* Too round for a card — use radius-lg (16px) */
```
...
*Mint Whisper — where Respiration meets Fraîcheur, Chaleur embraces Raffinement.*
