...
import { CrmSyncService } from '@process/services/ma/CrmSyncService';
    ...
    data: [
      {
        id: 'test-conn-id',
        providerId: 'hubspot',
        providerConfigKey: 'hubspot-config',
        status: 'connected',
        lastSyncedAt: 900000,
        createdAt: 800000,
        updatedAt: 900000,
...
describe('CrmSyncService - Readiness', () => {
  ...
  it('returns ready=false when no CRM connections exist', async () => {
    mockIntegrationConnectionRepo.list.mockResolvedValue({
      success: true,
      data: [],
    });
    ...
    expect(readiness.ready).toBe(false);
    expect(readiness.hasConnection).toBe(false);
    expect(readiness.error).toContain('No crm connections');
  });
  ...
  it('counts active jobs correctly', async () => {
    mockSyncJobRepo.countActiveJobs.mockResolvedValue({ success: true, data: 2 });
    const service = new CrmSyncService(deps);
    const readiness = await service.getReadiness();

    expect(readiness.activeJobs).toBe(2);
  });
});

describe('CrmSyncService - Sync Flow', () => {
  it('creates and starts a sync job successfully', async () => {
    const service = new CrmSyncService(deps);
    const job = await service.startSync({
      jobType: 'crm',
      providerId: 'hubspot',
      config: { objectTypes: ['contacts', 'deals'], limit: 100 },
    });

    expect(job.id).toBe('test-job-id');
    expect(job.jobType).toBe('crm');
    expect(job.providerId).toBe('hubspot');
  ...
  it('syncs multiple object types when configured', async () => {
    const service = new CrmSyncService(deps);
    const job = await service.startSync({
      jobType: 'crm',
      providerId: 'hubspot',
      config: { objectTypes: ['contacts', 'deals', 'companies'] },
    });
    ...
    const finalJob = await service.getJob(job.id);
    expect(finalJob?.status).toBe('completed');
    expect(finalJob?.itemsTotal).toBeGreaterThan(0);
  });
});

describe('CrmSyncService - Cancellation', () => {
...
describe('CrmSyncService - Job Management', () => {
  ...
  it('lists jobs with optional status filter', async () => {
    ...
    expect(mockSyncJobRepo.list).toHaveBeenCalledWith({ jobType: 'crm', status: 'completed' });
  });
...
describe('CrmSyncService - Failure State', () => {
